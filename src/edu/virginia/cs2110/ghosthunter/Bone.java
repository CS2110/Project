...
public class Bone {
	...
	/*
	 * public Bone (Ghost ghost, Hunter hunter){ this.ghost = ghost; this.hunter
	 * = hunter; x = rand.nextFloat() + ghost.getX(); // need to find out how
	 * large to make nextDouble y = rand.nextFloat() + ghost.getY(); // ^ }
	 * public double distance(Hunter hunter) { return Math.sqrt(Math.pow(x -
	 * hunter.getX(), 2) + Math.pow(y - hunter.getY(), 2)); } public float
	 * getX() { return x; } public void setX(float x) { this.x = x; } public
	 * float getY() { return y; } public void setY(float y) { this.y = y; }
